# Welcome to Maqam World

> Foundations of Arabic Music Theory - A Violin-Oriented Guide

This documentation provides a comprehensive introduction to Arabic music theory, specifically designed for violinists with a background in Western music theory.

## 📚 Course Structure

### Level 1: Foundation

- [Module 1: The Foundations of the Arabic Sound System](#module-1-the-foundations-of-the-arabic-sound-system)
- [Module 2: The Building Blocks — Ajnas](#module-2-the-building-blocks--ajnas-singular-jins)
- [Module 3: Constructing Full Scales (Maqamat)](#module-3-constructing-full-scales-maqamat)
- [Module 4: The Art of Melodic Movement (Sayr and Modulation)](#module-4-the-art-of-melodic-movement-sayr-and-modulation)

[Start Level 1 →](/en/level-1.md)


### Level 2: Advanced Scales (Maqamat) - Tetrachord and Pentachord

- [Module 5: Other Ajnas Types — Trichords and Pentachords](#module-5-other-ajnas-types--trichords-and-pentachords)
- [Module 6: Reading the Tradition — Arabic Note Names & Transposition](#module-6-reading-the-tradition--arabic-note-names--transposition)
- [Module 7: Maqam Families — The Big Map](#module-7-maqam-families--the-big-map)
- [Module 8: Survey of Common Maqamat](#module-8-survey-of-common-maqamat)

[Start Level 2 →](/en/level-2.md)

### Level 3: Coming Soon

Mastery and performance applications.

### Appendix B - Maqamat Scores

[Navigate Scores](/en/appendix/maqamt-score.md)

## 🌍 Languages

- [English Guide](/en/)
- [Arabic Guide](/ar/) (قريباً)

## 🎯 Prerequisites

Students should already have a solid foundation in Western music theory — intervals, major/minor scales, tone and semitone.

## 🎻 Instrument Focus

This course is designed for violin with standard Western tuning (G–D–A–E). No alternate tuning is used except where explicitly discussed for explanatory purposes.

## 📖 What You'll Learn

- The Arabic 24-quarter-tone system and how it relates to Western tuning
- Half-flat (𝄳) and half-sharp (𝄵) notation
- Neutral intervals (Sikah and Awj)
- Ajnas (melodic cells) - Rast, Bayati, Hijaz, and Nahawand
- Maqamat (full scales) and their construction
- Sayr (melodic movement) and modulation techniques
- Qafla (cadential formulas)

## 🔗 External Resources

For audio examples and additional reference:

- [MaqamWorld](https://www.maqamworld.com) - Comprehensive Arabic music theory resource

---

*Built with [Docsify](https://docsify.js.org)*
